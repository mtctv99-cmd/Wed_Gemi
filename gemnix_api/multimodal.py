...
def upload_image(image_bytes: bytes, filename: str = "image.png", mime_type: str = "image/png") -> str | None:
    """Upload image via Scotty resumable upload. Returns file reference path or None on failure."""
    ...
    try:
        ...
        log(f"Upload start failed: {e}")
        return None

    upload_url = resp.headers.get("X-Goog-Upload-URL") or resp.headers.get("x-goog-upload-url")
    if not upload_url:
        log(f"No upload URL in response headers: {dict(resp.headers)}")
        return None
